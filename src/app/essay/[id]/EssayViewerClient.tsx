...
export default function EssayViewerClient({ essayId }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const essays: Essay[] = JSON.parse(stored)
  ...
  function handleDelete() {
    ...
    try {
      ...
      if (stored) {
        ...
        localStorage.setItem('legebot_essays', JSON.stringify(updated))
      }
    } catch { /* ignore */ }
    router.push('/essays')
  }
  ...
  function handleShare() {
    if (!essay) return
    if (navigator.share) {
      navigator.share({ title: `Dissertation : ${essay.subject}`, text: essay.content })
    }
  }
  ...
  function formatContent(text: string) {
    ...
    return text.split('\n').map((line, i) => {
      ...
      if (isProblematique) {
        return (
          <p key={i} className="my-2 px-3 py-2 rounded-lg text-sm leading-relaxed italic font-medium"
            style={{ background: 'rgba(245,166,35,0.08)', borderLeft: '3px solid #F5A623', fontFamily: "'Courier New', Courier, monospace", color: '#F0F0F5' }}>
            {line}
          </p>
        )
      }
      return (
        <p key={i} className="text-sm text-[#E8E8F0] leading-[1.9]"
  ...
  if (!essay) {
    return (
      <div className="min-h-screen bg-[#0E0F14] flex items-center justify-center flex-col gap-4">
        <div className="text-4xl">📄</div>
        <p className="text-[#9A9BB0] text-sm">Dissertation introuvable</p>
        <button onClick={() => router.push('/essays')}
          className="text-[#F5A623] text-sm hover:underline">← Retour</button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#0E0F14] flex flex-col max-w-lg mx-auto">
      <div className="flex items-center gap-3 px-4 pt-12 pb-4 border-b border-[rgba(255,255,255,0.07)]">
        <button onClick={() => router.back()} className="text-[#9A9BB0] hover:text-[#F0F0F5] transition-colors">
          <ArrowIcon />
        </button>
        ...
        <button onClick={handleShare} className="text-[#9A9BB0] hover:text-[#F0F0F5] transition-colors">
          <ShareIcon />
        </button>
      </div>
      ...
      <div className="mx-4 mt-4">
        <div className="bg-[#1E2030] rounded-xl border border-[rgba(245,166,35,0.3)] p-4">
          ...
          <p className="text-[#F0F0F5] text-sm italic" style={{ fontFamily: "'Courier New', Courier, monospace" }}>
            {essay.subject}
          </p>
          {essay.score && (
